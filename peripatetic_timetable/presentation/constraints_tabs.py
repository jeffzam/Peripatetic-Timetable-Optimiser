...
class LocksTab(_ManagedTab):
    def __init__(self, parent, add_callback, delete_callback):
        ...
        ttk.Label(form, text="Teacher locks", style="Title.TLabel").grid(row=0, column=0, columnspan=5, sticky="w", pady=(0, 10))
        self.teacher, self.school = ttk.Combobox(form, state="readonly", width=25), ttk.Combobox(form, state="readonly", width=18)
        self.day = ttk.Combobox(form, state="readonly", values=DAYS, width=14)
        for column, (label, widget) in enumerate((("Teacher", self.teacher), ("Day", self.day), ("Must remain at", self.school))):
            ttk.Label(form, text=label, style="Card.TLabel").grid(row=1, column=column, sticky="w")
            widget.grid(row=2, column=column, padx=(0, 10))
        buttons = ttk.Frame(form, style="Card.TFrame"); buttons.grid(row=2, column=3); self._buttons(buttons)
    ...
    def values(self):
        return self.teacher.get(), self.day.get(), self.school.get()
